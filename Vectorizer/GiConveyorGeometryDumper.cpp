...
void OdGiConveyorGeometryDumper::polylineOut(OdInt32 numPoints, const OdGePoint3d* vertexList)
{
  ...
  m_pDumper->output(OD_T("End polylineOut"));
	...
	{
		pList.Add(CPnt(vertexList[i].x, vertexList[i].y, vertexList[i].z));
	}
	CPrimPolyline* pPolyline = new CPrimPolyline(pList);
	CPegDoc::GetDoc()->GetDoc()->WorkLayerGet()->AddTail(new CSeg(pPolyline));
}
...
// DD does not call this method in 1.13
/************************************************************************/
