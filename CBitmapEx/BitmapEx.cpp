...
float CBitmapEx::pm_ARG(float xa, float ya)
{
	float _arg = 0.0f;
...
_PIXEL CBitmapEx::_RGB2HSV(_PIXEL rgbPixel)
{
	...
	float _hue = pm_ARG(xa, ya) * (180.0f / _PI) + 150.0f;
	float _saturation = pm_ARG(temp, _MOD((float)(red-temp), (float)(green-temp), (float)(blue-temp))) * 100.0f / atan(sqrt(6.0f));
